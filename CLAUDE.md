...
python apps/core/notebook_parser.py "ClassLib/Python基础程序设计/第一课_基本数据结构.ipynb"
# Full structured JSON / section-grouped / learning-app Cell payloads
python apps/core/notebook_parser.py "ClassLib/Python基础程序设计/第一课_基本数据结构.ipynb" --format json|sections|cells
...
- **Shared parser**: `apps/core/notebook_parser.py` (pure stdlib, no Django dependency) is the single source of truth for parsing `ClassLib/<category>/*.ipynb` materials (category directories, scanned recursively). Both management commands and the Claude Code skill `.claude/skills/notebook-reader/` use it.
