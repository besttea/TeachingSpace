...
- **验收**：教师工作台可见覆盖矩阵;缺口一键出题端到端测试 ✅（课程管理页知识点行显示习题/考题/提交/通过率徽标，「未出题」红标一键跳转练习创建页预选该知识点）

### 1.2 考试按知识点勾选出题 ✅
- **现状**：关联课程后整卷生成自动用全部知识点,教师无法控制范围(如期中只考前 5 章)。
- **方案**：AI 生成对话框改为「勾选知识点(默认全选)+ 每知识点题数提示」;生成时只分发所选知识点;所选数不足题数时自动允许重复并提示。
- **验收**：勾选子集生成只覆盖所选知识点(回归测试) ✅（生成对话框改为 Bootstrap 弹窗：知识点复选框全选/清空 + 题数/难度；服务端校验知识点归属，越权 id 400）
...
- **验收**：映射规则单测 + 页面展示 ✅（`apps/training/mastery.py` 纯函数：练习最近一次提交 + 已交卷考题正确性 → 未开始/学习中(<70%)/已掌握；「我的进度」页按课程分组渲染进度条与计数；跨学生数据隔离测试）

### 2.2 薄弱知识点一键练 ✅
- **现状**：学生知道薄弱点后无处下手。
- **方案**：掌握度视图每条薄弱知识点带「去练习」——跳转该知识点绑定的练习列表(无绑定练习时推荐 AI 生成?不——学生无生成权限,显示「暂无练习,请向教师反馈」)。
- **验收**：跳转与空状态 ✅（掌握度视图「去练习」→ 练习列表 `?knowledge_point=` 筛选横幅；无配套练习时显示「请向教师反馈」空状态）
...
| T38 | 学生无掌握度视图 | 练习/考试数据未映射到知识点 | P0 | ✅ 2.1（含 2.2） |
...
- 2026-09-17 ｜ v3 冲刺①·知识点闭环 ｜ **覆盖矩阵**（教师工作台知识点行显示习题/考题/提交/通过率徽标，未出题红标一键跳练习创建页预选）；**考试按知识点勾选出题**（弹窗勾选/全选清空，服务端归属校验）；**学生知识点掌握度**（练习最近提交+考题正确性映射，未开始/学习中/已掌握 + 进度条）；**薄弱一键练**（?knowledge_point= 筛选横幅与空状态反馈） ｜ 393 |
